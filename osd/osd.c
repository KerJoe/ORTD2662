...
//    +-------------+-------------+-------------+ -- 1
//    | Byte 1, LSN | Byte 0, MSN | Byte 0, LSN |
//    +-------------+-------------+-------------+ -- 2
//    | Byte 5, MSN | Byte 5, LSN | Byte 4, MSN |
//    +-------------+-------------+-------------+ -- 3
//    | Byte 4, LSN | Byte 3, MSN | Byte 3, LSN |
//    +-------------+-------------+-------------+ -- 4
//    |                   ...                   |
//    +-------------+-------------+-------------+ -- 18
//    |             |             |             |
...
void UploadFont_1Bit_8x16(uint8_t font[][16])
{
    ...
    XSFRWriteByte(SCALER_ADDRESS, S_OSD_PORT);
    for (uint16_t i = 0; i < 128; i++)
        for (uint8_t k = 1; k < 19; k+=2)
        {
            XSFRWriteByte(SCALER_DATA, font[i][k] << 4);
            XSFRWriteByte(SCALER_DATA, font[i][k] >> 4);
            XSFRWriteByte(SCALER_DATA, font[i][k-1]);
        }
}
