...
BYTE CIRKeyScan(void)
{
	...
	ucIrdaStatus = XSFRReadByte(MCU_IR_STATUS_FF89);
...
{
	XSFRWriteByte(MCU_IR_CTRL1_FF80, 	tIrdaInitial[0]);
	XSFRWriteByte(MCU_IR_DEBOUNCE_FF81, 	tIrdaInitial[1]);
	XSFRWriteByte(MCU_IR_BURST_LENGTH_FF82, 	tIrdaInitial[2]);
	XSFRWriteByte(MCU_IR_SILENCE_LENGTH_FF83, 	tIrdaInitial[3]);
	XSFRWriteByte(MCU_IR_REPEAT_LENGTH_FF84, 	tIrdaInitial[4]);
	XSFRWriteByte(MCU_IR_MOD_LENGTH_FF85, 	tIrdaInitial[5]);
	XSFRWriteByte(MCU_IR_DATA0_LENGTH_FF86, 	tIrdaInitial[6]);
	XSFRWriteByte(MCU_IR_DATA1_LENGTH_FF87, 	tIrdaInitial[7]);

	XSFRWriteByte(MCU_IR_CTRL2_FF88, 	tIrdaInitial[8]);

	XSFRWriteByte(MCU_IR_ONE_CMD_TIME_FF90, 	tIrdaInitial[9]);
   	XSFRWriteByte(MCU_IR_IDLE_TIME_FF91, 	tIrdaInitial[10]);
 	XSFRWriteByte(MCU_IR_CTRL3_FF92, 	tIrdaInitial[11]);

	XSFRWriteByte(MCU_IR_INTRQ_FF93, 	0x00);//0x00;
	XSFRWriteByte(MCU_IR_STATUS_FF89, XSFRReadByte(MCU_IR_STATUS_FF89) | _BIT0);//|_BIT1;//enable IR
}

static void CIrdaReadData(void)
{
	ucIrQueue[0] =	XSFRReadByte(MCU_IR_DATA2_FF8C);
	ucIrQueue[1] =	XSFRReadByte(MCU_IR_DATA3_FF8D);
