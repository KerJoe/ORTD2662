...
void InitHDMI()
{
    ...
    CSourceScanInputPortDVI(1);


    ScalerWriteByte(S_PAGE_SELECT, 2);
    ScalerWriteBit(S2_TMDS_MEAS_RESULT0, 1, 0b1);     // Select Port 1 ???
    ScalerWriteByte(S2_POWER_ON_OFF_CONTROL, 0x4F);   // Swap B/R Channels and swap P/N, Input channel control is manual, Enable input Clock and RGB port
    ScalerWritePortBit(S2_HDMI_PORT, SP2_HDMI_AVMCR, 3, 0b1); //Enable DVI/HDMI video output
